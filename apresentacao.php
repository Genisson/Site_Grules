...
if (!$conn->connect_error){
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			...
			<a href='cadastro-apresentacao.php?operacao=2&id=" .  $row["apresentacao_id"] . "'><i class='fas fa-edit' name='editar'></i></a>
			</td>
			<td style='text-align: right;'>
			<a href='delete-apresentacao.php?id=" .  $row["apresentacao_id"] . "'><i style='color: red;' class='fas fa-trash' name='deletar'></i></a>
			</td>
			</tr>
			<td style='text-align: left;'>
			<a href='todosalunos.php?id=" .  $row["apresentacao_id"] . "'><i style='color: green;' class='fas fa-plus' name='adicionar'></i></a>
			</td>
			</tr>";
		}
	} else {
